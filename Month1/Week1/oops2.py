...
class Organization:
    ...
    def isEligibleForBonus(self, overTimeThreshold):
        for i in self.employee_list:
            ...
            for value in i.overtimeContribution.values():
                totalOvertime += value
    ...
    def totalBonus(self, ratePerHour):
        ...
        for obj in self.employee_list:
            if obj.overTimeStatus:
                for value in obj.overtimeContribution.values():
                    total += value * ratePerHour
...
for i in range(n):
    ...
    obj_list.append(
        Employee(employee_name, designation, salary, overTimeContribution)
    )
...
print(org_obj.totalBonus(ratePerHour))